...
export function ClipViewer({ clipId, onClose }: ClipViewerProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${clip.title.replace(/[^a-z0-9\-_. ]/gi, "_").slice(0, 120) || "clip"}.mp4`;
      ...
      <div className="flex items-center justify-between p-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-sm text-white/70 hover:text-white hover:bg-white/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-white/70"
          >
            {downloading ? (
